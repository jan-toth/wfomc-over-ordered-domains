...
class DimacsCNF:
    ...
    def __init__(self):
        ...
        # Track ONLY the auxiliary variables introduced by CC encoders (to be excluded)
        self.auxiliary_vars: Set[int] = set()
    ...
    def add_cardinality_constraint(self, literals: List[int], op: str, bound: int):
        ...
        Any new variables introduced here are marked as auxiliary.
        """
        previous_nv = self.num_vars

        # WARNING: pysat CCs may not be equicountable
        # which is why we need to employ projected model counting
        if op == '<=':
            cnf_obj = CardEnc.atmost(literals, bound=bound, top_id=previous_nv, encoding=EncType.seqcounter)
        elif op == "<":
            cnf_obj = CardEnc.atmost(literals, bound=bound - 1, top_id=previous_nv, encoding=EncType.seqcounter)
        elif op == '>=':
            cnf_obj = CardEnc.atleast(literals, bound=bound, top_id=previous_nv, encoding=EncType.seqcounter)
        elif op == ">":
            cnf_obj = CardEnc.atleast(literals, bound=bound + 1, top_id=previous_nv, encoding=EncType.seqcounter)
        elif op == '=' or op == '==':
            cnf_obj = CardEnc.equals(literals, bound=bound, top_id=previous_nv, encoding=EncType.seqcounter)
        else:
            raise ValueError(f"Unsupported operator: {op}")

        self.clauses.extend(cnf_obj.clauses)

        # If new variables were created for CC encoding, we update the counter and mark them as auxiliary
        if cnf_obj.nv > self.num_vars:
            self.num_vars = cnf_obj.nv
            
            new_vars = range(previous_nv + 1, self.num_vars + 1)
            self.auxiliary_vars.update(new_vars)


    def write_to_file(self, filename: str, metadata=False):
        """
        Writes the CNF to a file.
        If auxiliary variables exist, it calculates the projection set (All - Aux) and writes it to the CNF header.
        """
        ...
        # 1. Calculate Projection Set
        has_projection = len(self.auxiliary_vars) > 0
        if has_projection:
            all_vars = set(range(1, self.num_vars + 1))
            proj_list = sorted(list(all_vars - self.auxiliary_vars))

        # 2. Write Main CNF file
        with open(filename, 'w') as f:
            if metadata:
                for (atom, id_) in self.atom_to_id.items():
                    f.write(f"c {atom}: {id_}\n")

            # Header
            f.write(f"p cnf {self.num_vars} {len(self.clauses)}\n")

            # Only write 'c ind' if we have auxiliary variables (if we want PMC)
            if has_projection:
                # f.write(f"c ind {' '.join(map(str, proj_list))} 0\n")
                chunk_size = 50 
                for i in range(0, len(proj_list), chunk_size):
                    chunk = proj_list[i:i + chunk_size]
                    f.write(f"c ind {' '.join(map(str, chunk))} 0\n")
...
def ground(input: str, out_file: str, check_order_preds=True):
    prob = parse_input(input)
    cnf = _ground_wfomc(prob, check_order_preds)
    cnf.write_to_file(out_file)


def parse_args():
    parser = argparse.ArgumentParser(
        description='Generate CNF files for .wfomcs or .mln',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    
    parser.add_argument("--walk", "-w", type=str, required=False, help="folder to walk for WFOMC problems", default=None)
    parser.add_argument("--output_dir", "-od", type=str, required=False, help="folder to store generated CNFs", default=None)

    parser.add_argument('--input', '-i', type=str, required=False, help='singular problem file', default=None)
    parser.add_argument('--output_file', '-of', type=str, required=False, help="filename to storethe generated CNF", default=None)
...
if __name__ == '__main__':
    ...
    if args.walk:
        ...
        paths = [input_dir.joinpath(f) for f in natsorted(os.listdir(input_dir)) if (os.path.isfile(input_dir.joinpath(f)))]
        for path in paths:
            name, ext = os.path.splitext(path)
            if ext != ".wfomcs" and ext != ".mln":
                continue
            
            out_file = f"{output_dir.joinpath(name.split(os.path.sep)[-1])}.cnf"
            ground(str(path), out_file)
