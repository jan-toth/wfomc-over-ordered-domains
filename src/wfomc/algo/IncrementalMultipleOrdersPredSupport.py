...
def incremental_multiple_orders_wfomc(context: WFOMCContext, t) -> RingElement:
    return _incremental_wfomc_with_predecessor(context)
...
def incremental_multiple_orders_wfomc_old(context: WFOMCContext, t) -> RingElement:
    ...
    evidence = dict()
    evidence["SUC type 1 with PRED"] = frozenset((
        AtomicFormula(successor_pred, (a, b), False),
        AtomicFormula(successor_pred, (b, a), False),
        AtomicFormula(predecessor_preds[1], (b, a), True),
    ))
    evidence["SUC type 1 without PRED"] = frozenset((
        AtomicFormula(successor_pred, (a, b), False),
        AtomicFormula(successor_pred, (b, a), False),
        AtomicFormula(predecessor_preds[1], (b, a), False),
    ))
    evidence["SUC type 2 with PRED"] = frozenset((
        AtomicFormula(successor_pred, (a, b), False),
        AtomicFormula(successor_pred, (b, a), True),
        AtomicFormula(predecessor_preds[1], (b, a), True),
    ))
    evidence["SUC type 2 without PRED"] = frozenset((
        AtomicFormula(successor_pred, (a, b), False),
        AtomicFormula(successor_pred, (b, a), True),
        AtomicFormula(predecessor_preds[1], (b, a), False),
    ))
    evidence["SUC type 3 with PRED"] = frozenset((
        AtomicFormula(successor_pred, (a, b), True),
        AtomicFormula(successor_pred, (b, a), False),
        AtomicFormula(predecessor_preds[1], (b, a), True),
    ))
    evidence["SUC type 3 without PRED"] = frozenset((
        AtomicFormula(successor_pred, (a, b), True),
        AtomicFormula(successor_pred, (b, a), False),
        AtomicFormula(predecessor_preds[1], (b, a), False),
    ))
            ...
            for j, cell in enumerate(cells):
                ...
                for key, w_old in old_table.items():
                    ...
                    for k, other_cell in enumerate(cells):
                        if other_cell == last_cell:
                            w_new = (
                                w_new * cell_graph.get_two_table_weight((cell, other_cell), evidence=evidence["with PRED"])
                                * cell_graph.get_two_table_weight((cell, other_cell), evidence=evidence["without PRED"]) ** max(ivec[k] - 1, 0)
                            )
                        else:
                            w_new = w_new * cell_graph.get_two_table_weight((cell, other_cell), evidence=evidence["without PRED"]) ** ivec[k]
                    ivec = list(ivec)
                    ivec[j] += 1
                    ivec = tuple(ivec)
                    w_new = w_new + table.get((ivec, cell), Rational(0, 1))
...
def _incremental_wfomc_with_predecessor_and_successor(context: WFOMCContext):
    #cells need to have their ids already fixed
    formula = context.formula
    domain = context.domain
    get_weight = context.get_weight
    leq_pred = context.leq_pred
    ...
    for order, pred in predecessor_preds.values():
        if order != 1:
            print("predecessor_preds of order higher than 1 are not supported with successor!")
            exit(1)
    ...
    def tail(h_old, sigma, rho, cell, m):
        ...
        for a, cell_a in enumerate(cells):
            for b, cell_b in enumerate(cells):
                ...
                lambda_suc_is_pred = cell_graph.get_two_table_weight((cell_b, cell), evidence=evidence["SUC type 2 with PRED"]) * \
                    (cell_graph.get_two_table_with_suc_weight((cell_b, cell), suc_type=1) ** (sigma[b] - 1)) * \
                    math.prod(1 if e == b else cell_graph.get_two_table_with_suc_weight((cell_s, cell), suc_type = 1) ** sigma[e] \
                    for e, cell_s in enumerate(cells))
                lambda_suc_is_not_pred = 1
    ...
    def only(h_old, sigma, rho, cell, m, pred):
        out = []
        W_cell = cell_graph.get_cell_weight(cell)
        lambda_ = math.prod(cell_graph.get_two_table_weight((cell_s, cell), evidence=evidence["SUC type 1 without PRED"]) ** sigma[e] \
                    if cell_s != pred else \
                    (cell_graph.get_two_table_weight((cell_s, cell), evidence=evidence["SUC type 1 without PRED"]) ** sigma[e - 1]) * \
                         cell_graph.get_two_table_weight((cell_s, cell), evidence=evidence["SUC type 1 with PRED"]) \
                    for e, cell_s in enumerate(cells))
        new_rho = list(rho)
        new_rho[m * n_cells + m] += 1
        new_sigma = list(sigma)
    ...
    for i in range(1, domain_size - (0 if first_cells is None else 1)):
        old_table = table
        table = dict()
        if i >= (domain_size + 1) // 2:
            max_rho_size = domain_size - i
    ...
    print(sum(table.values()))
    if first_cells is not None and domain_size > 1:
        max_rho_size = 1
        old_table = table
        table = dict()
        for j, cell in first_cells:
            ...
            for key, h_old in old_table.items():
                ...
                if rho_size <= max_rho_size:
                    ...
                    for new_key, h_new in h_news:
                        table[new_key] = table.get(new_key, Rational(0, 1)) + h_new
    print(sum(table.values()))
    return weight * sum(table.values())